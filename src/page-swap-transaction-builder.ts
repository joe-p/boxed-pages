...
import type { AppClientParams } from "@algorandfoundation/algokit-utils/app-client";
 ...
 * 2. Gets the method selector from the page spec
 * 3. Creates an updateApplication transaction via the page client
...
export async function buildSwapTransaction(
  ...
  const { algorand, targetPage, appId, sender } = params;
  ...
  // Get the method selector for the target page's primary method
  // We need to determine which method we're swapping to
  // The selector determines which page gets loaded
  const primaryMethod = getPrimaryMethod(targetPage.spec);
  const selector = getMethodSelector(targetPage.spec, primaryMethod);

  // Create the update transaction via the page client
  // This will use the target page's bytecode but update the current app
